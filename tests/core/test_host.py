...
def test_user_data_dir_default(monkeypatch, tmp_path):
    ...
    assert p == tmp_path / ".config" / "meti"


def test_user_data_dir_xdg_override(monkeypatch, tmp_path):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path / "xdg"))
    p = host.user_data_dir()
    assert p == tmp_path / "xdg" / "meti"


def test_vault_paths(monkeypatch, tmp_path):
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    monkeypatch.setenv("HOME", str(tmp_path))
    assert host.vault_path() == tmp_path / ".config" / "meti" / "credentials.json.age"
    assert host.vault_key_path() == tmp_path / ".config" / "meti" / "age-key.txt"
...
def test_user_providers_dir(monkeypatch, tmp_path):
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    monkeypatch.setenv("HOME", str(tmp_path))
    assert host.user_providers_dir() == tmp_path / ".config" / "meti" / "providers"


def test_settings_path(monkeypatch, tmp_path):
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    monkeypatch.setenv("HOME", str(tmp_path))
    assert host.settings_path() == tmp_path / ".config" / "meti" / "settings.toml"
